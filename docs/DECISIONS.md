...
| 2026-08-24 | Derive Mainnet hiring exposure from current release qualification | Approved, pre-write | Environment flags configure an already-qualified integration but cannot create hireability; landing, `/hire`, demo UI, quote and prepare require a current `qualified` readiness snapshot entry | Before proof publication, the hourly monitor treats a `404` as intentionally unavailable. Once the canonical Mainnet proof exists, a `404` fails; an exposed route requires the public `MAINNET_UPTIME_BUYER_ADDRESS` and verifies Agent Card, signed quote and read-only prepare without signing or broadcasting a transaction |
