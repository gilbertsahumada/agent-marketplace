...
## 2026-09-03 — Hire event telemetry is budgeted per caller; chain phases are not

- Reverses the 2026-09-01 non-goal "no distributed rate limiter on the same-origin route", for telemetry only. `clicked` rows are the one write a browser can add without an RPC-verified receipt, so they are the one write that needs a ceiling. Chain phases stay unbudgeted on purpose: their key is `chainId:txHash:phase` and every one costs the Worker a verified receipt, which bounds them by construction.
- The mechanism is the catalog-validation precedent (2026-08 migration 0017), not a new one: the marketplace derives the caller context (first forwarded address and origin) and forwards only its HMAC-SHA256 fingerprint under `x-marketplace-caller`, keyed by the shared buyer-observation secret with a purpose prefix (`hire-event-caller`) distinct from the validation one, so the two budgets never share a bucket. The Worker requires the header on `POST /hire-events` (missing or malformed is `400`, never an anonymous bucket), stores it as `hire_events.callerKey` (migration `0021`, default `anonymous` for older rows, index `(callerKey, provenance, occurredAt DESC)`), counts the caller's `marketplace_observed` rows since the UTC day start and answers `429` with `retry-after` past `HIRE_EVENTS_PER_CALLER_DAY` (Free 20 / Paid 200; maxima 200 / 2,000). The marketplace maps that answer to `MarketplaceRateLimitError` → `429` + `Retry-After` on the same-origin route.
- Shared code moved rather than copied: `callerKey()` into `bnb-agent-probe/src/lib/caller-key.ts`, `callerContext()` into `src/presentation/http/marketplace-http.ts`, and the fingerprint into `src/data/observation/caller-fingerprint.ts` with the purpose as a parameter (the validation fingerprint bytes are unchanged). Deployment order: Worker migration and deploy first; the marketplace that sends the header follows.

## 2026-09-03 — WP7 release gate: a checklist and a read-only smoke, not a judgement call

- `docs/RELEASE.md` turns the WP7 gate of the spec (tests, production build, staging smoke on and off, kill switch, observability and rollback, only then a production Cron) into commands with expected results, in order. It creates no infrastructure and names no secret value; the production steps that only the account owner can perform (create the production D1 and Queue, install secrets, point the marketplace at the production Worker, enable the Cron in a final commit) are listed as operator steps, and until they land the live observation data plane remains the Paid staging Worker reached through `OBSERVATIONS_URL`.
- `npm --prefix bnb-agent-probe run smoke -- <worker-origin> <marketplace-origin> [--expect-kill-switch 0|1]` is the smoke: GETs only, against the public Worker surfaces (`/health`, `/catalog-agents`, `/hire-events`) and the marketplace API (agents, passport, Testnet job), one line per target and a non-zero exit when a status, a payload shape or the expected kill-switch state disagrees. The plan builder and the evaluator are pure functions covered by unit tests, following `rollback-wp2-activation.ts`; the script never posts, never carries a bearer and never reads D1 directly, so it can run against production at any time without entering the evidence it measures.

## 2026-09-03 — P6 implemented: one confirmation through EIP-5792, sequential fallback, no permit
