...
| 2026-08-28 | Keep WP2 cron disabled after real Free CPU measurements | Active blocker | Cloudflare Analytics measured HEADER=25 at 15,442 µs and SWEEP=4 at 11,098 µs; limit-one HEADER still measured 17,124 µs in preview and 11,676 µs through the authenticated nominal route, above the 10,000 µs Free limit | Staging remains `KILL_SWITCH=1` with an empty schedule list; configuration stays parameterized, but lowering limits alone does not pass the CPU gate. Do not activate cron or Paid until WP2 is optimized and remeasured or payment is explicitly approved |
