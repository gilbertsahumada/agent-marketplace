...
| 2026-08-29 | Recalculate the Free D1 query envelope after the WP4 ORM migration | Active | Drizzle preserves schema safety but emits one statement per target write; a SWEEP page of 40 agents with two endpoints could no longer fit the 40-query executable ceiling | `SWEEP_LIMIT` is capped at 13 and `D1_QUERIES_PER_RUN` has a Free minimum of 22. Config rejects combinations whose two-target-per-agent SWEEP worst case, phase state, lease and four cleanup/ledger reserves exceed the configured budget; the default remains 4/40 |
