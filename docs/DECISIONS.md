...
| 2026-08-19 | Replace the unrecoverable local seller with a public hosted Testnet fixture | Approved for Gate 6A | Agent `1815`'s keystore password was never retained, so its signing key cannot be recovered; a new Testnet-only key is held as a server-side Vercel secret and public A2A routes keep the fixture usable without a tunnel | Requires a new ERC-8004 Agent ID after the stable endpoint is deployed; deterministic deliverables avoid adding storage, and the browser spike remains disabled until registration completes |
