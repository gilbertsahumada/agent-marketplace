...
import type { HireChainId, HireJobPage, HireLedgerCounts, HireLedgerSummary } from "@/src/business/entities/hire-job";
...
function jobsHref(chainId: HireChainId, before?: string): string {
  return before === undefined ? `/jobs?chainId=${chainId}` : `/jobs?chainId=${chainId}&before=${before}`;
}

// Marketplace-wide ledger of indexed ERC-8183 jobs. Two numbers on purpose:
// the protocol's total and the subset with a chain-verified hire event. Both
// are activity counts; a settled job proves the phase, not the deliverable.
export function HireLedgerPage({ chainId, summary, page, before }: {
  chainId: HireChainId;
  ...
  return (
    <main className="mx-auto w-full max-w-6xl flex-1 px-4 py-10 sm:px-6 lg:px-8 lg:py-14" id="main-content">
      ...
      {summary === null && page === null ? (
        <div className="mt-8 rounded-xl border border-amber-400/20 bg-amber-400/5 px-5 py-6">
          ...
          <Button asChild className="mt-4" variant="outline">
            <Link href={jobsHref(chainId, before)}><RotateCcw aria-hidden="true" />Try again</Link>
          </Button>
        </div>
      ) : null}
      ...
      {summary !== null ? (
        <>
          <div className="mt-8 grid gap-4 sm:grid-cols-2">
            <CountsCard counts={summary.protocol} note="Every Commerce job the indexer has seen on this network." title="Protocol" />
            <CountsCard counts={summary.marketplace} note="Jobs with a chain-verified hire event recorded by this marketplace." title="Processed through this marketplace" />
          </div>
          <p className="mt-3 text-xs text-zinc-500">
            ...
            {summary.lastIndexRun ? ` · last run ${summary.lastIndexRun.status}` : ""}
          </p>
        </>
      ) : null}
      ...
      {page !== null ? (
        <section aria-labelledby="recent-jobs" className="mt-8 rounded-xl border border-white/10 bg-white/[0.015]">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-white/10 px-4 py-4 sm:px-5">
            <h2 className="text-base font-medium text-white" id="recent-jobs">{before === undefined ? "Recent jobs" : `Jobs before #${before}`}</h2>
            <span className="text-sm text-zinc-500">{page.jobs.length} shown</span>
          </div>
          <HireJobRows chainId={chainId} emptyText="No indexed jobs on this network yet." jobs={page.jobs} />
          {page.nextBefore !== null ? (
            <div className="border-t border-white/10 px-4 py-4 sm:px-5">
              <Button asChild variant="outline">
                <Link href={jobsHref(chainId, page.nextBefore)}>Older jobs<ArrowRight aria-hidden="true" /></Link>
              </Button>
            </div>
          ) : null}
        </section>
      ...
      <p className="mt-6 text-xs text-zinc-600">Other network: <Link className="underline-offset-4 hover:underline" href={jobsHref(otherChain)}>{otherChain === 56 ? "BSC Mainnet" : "BSC Testnet"}</Link></p>
    </main>
  );
}
