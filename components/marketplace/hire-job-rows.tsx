...
import { CopyJobId, JobTimestamp } from "./job-table-metadata";
...
export function HireJobTableRow({ job }: { job: Pick<HireJob, "jobId" | "chainId" | "buyer" | "provider" | "status" | "updatedAt" | "registeredAt" | "marketplace"> }) {
  return <TableRow>
    <TableCell className="px-5 py-4 font-medium"><CopyJobId jobId={job.jobId} />{job.marketplace ? <span className="block text-xs font-normal text-muted-foreground">Hired here</span> : null}</TableCell>
    ...
    <TableCell className="px-5 text-xs text-muted-foreground"><JobTimestamp value={job.registeredAt ?? null} /></TableCell>
    <TableCell className="px-5 text-xs text-muted-foreground"><JobTimestamp value={job.updatedAt} /></TableCell>
