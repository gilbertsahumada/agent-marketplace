...
export function JobClosureActions({ report, refresh, network = "mainnet" }: { report: ClosureSummary; refresh: () => void; network?: "mainnet" | "testnet" }) {
  ...
  if (!address || !connector) return <p className="text-sm text-muted-foreground">{report.refundAvailable ? "Connect the buyer wallet to withdraw this deposit." : "Connect your wallet to review closure actions."}</p>;
  ...
  if (report.refundAvailable && report.buyer?.toLowerCase() !== address.toLowerCase()) return <p className="text-sm text-muted-foreground">Connect the original buyer wallet to claim the refund.</p>;
...
function ClosureControls({ report, wallet, getProvider, refresh, network }: { report: ClosureSummary; wallet: string; getProvider: () => Promise<unknown>; refresh: () => void; network: "mainnet" | "testnet" }) {
  ...
  const refund = network === "testnet" && report.refundAvailable === true;
  const action: ClosureAction | null = refund ? "refund" : report.closure === "review_window" ? "dispute" : report.closure === "settlement_available" ? "settle" : null;
  async function run(selected: ClosureAction, mode: "send" | "resume") {
    ...
    try {
      ...
      if (selected === "refund" && result.state === "confirmed") setMessage("Refund confirmed on-chain.");
  ...
  return <fieldset className="space-y-3 border-t border-border pt-4" disabled={busy} aria-busy={busy}>
    ...
    <details><summary className="cursor-pointer text-sm text-muted-foreground">Previous transactions</summary><div className="mt-3 flex flex-wrap gap-2">
      ...
      {!refund && <><button className={button} onClick={() => void run("dispute", "resume")}>Check previous dispute transaction</button>
