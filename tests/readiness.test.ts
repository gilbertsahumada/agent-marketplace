...
function assessorOptions(fetchImpl: typeof fetch) {
  ...
  return {
    ...
    createQuoteContext: async () => ({
      ...
      blockNumber: 123n,
      blockTimestamp: 1_949_999_900n,
      onchainObservedAt: "2031-10-17T10:38:20.000Z",
      ...
      quote: {
        ...
        contractContext: {
          ...
          onchainChecks: {
            ...
            policyAllowlisted: true,
            blockNumber: "123",
            blockTimestamp: { unix: "1949999900", iso: "2031-10-17T10:38:20.000Z" },
            observedAt: "2031-10-17T10:38:20.000Z",
...
describe("ERC-8183 readiness protocols", () => {
  ...
  it("stops probing when the run wall-clock budget is exhausted", async () => {
    ...
    const result = await createHireabilityAssessor({
      ...
      monotonicNow,
      probeBudget: createProbeBudget({
        maxMcpEndpoints: 0,
        maxSellerEndpoints: 48,
        maxTotalEndpoints: 48,
  ...
  it("shares bounded MCP and seller quotas under one execution deadline", () => {
    let elapsed = 0;
    ...
    expect(Array.from({ length: 24 }, () => budget.claim("mcp")).every((claim) => claim.allowed)).toBe(true);
    expect(budget.claim("mcp").allowed).toBe(false);
    expect(Array.from({ length: 48 }, () => budget.claim("seller")).every((claim) => claim.allowed)).toBe(true);
    expect(budget.claim("seller").allowed).toBe(false);
    elapsed = 180_001;
...
describe("Gate 1 proof", () => {
  ...
  it("keeps a completed job valid after it has passed through submitted", async () => {
    const proof = await verifyGate1Proof(
      gate1Reader({ status: JobStatus.COMPLETED }),
      () => 1_776_643_200_000,
    );
    expect(proof).toMatchObject({
      status: "verified",
...
function trustFixtureFetch(
  ...
  return (async (input: string | URL | Request) => {
    ...
    if (url.pathname === "/api/app/agents/profile") return Response.json(profiles[url.searchParams.get("agentId") ?? ""]);
...
describe("marketplace readiness report", () => {
  ...
  it("expires a verified quote at finalization without deleting its historical evidence", async () => {
    const [list, profiles, scores, onchain] = await Promise.all([
      fixture("trust8004/list.json"),
      fixture("trust8004/profiles.json") as Promise<Record<string, unknown>>,
      fixture("trust8004/scores.json") as Promise<Record<string, unknown>>,
    ...
    const report = await buildBscMarketplaceReadinessReport({
      ...
      assessHireability: async (candidate) => candidate.agentId === "45650"
        ? quoteVerifiedAssessment(onchain.agents["45650"]!.agentWallet)
        : mcpOnly,
      now: () => timestamps[Math.min(nowIndex++, timestamps.length - 1)]!,
    });

    const candidate = report.candidates.find((entry) => entry.agentId === "45650")!;
    expect(report.generatedAt).toBe("2031-10-17T10:50:01.000Z");
    expect(candidate.activation).toMatchObject({ quoteStatus: "expired", hireability: "expired_quote" });
    expect(candidate.activation.protocols[0]).toMatchObject({
      status: "quote_expired",
      quoteStatus: "expired",
      quote: { negotiationHash: `0x${"c".repeat(64)}` },
    });
    expect(candidate.qualification).toMatchObject({ status: "not_qualified", reasons: ["QUOTE_EXPIRED"] });
    expect(report.activationCoverage.quoteVerifiedAgentIds).toEqual([]);
    expect(report.sellerQualification.status).toBe("attention_required");
  });

  it("keeps manifest categories canonical when profile-derived classification drifts", async () => {
    const [list, sourceProfiles, scores, onchain] = await Promise.all([
      fixture("trust8004/list.json"),
      fixture("trust8004/profiles.json") as Promise<Record<string, unknown>>,
      fixture("trust8004/scores.json") as Promise<Record<string, unknown>>,
      fixture("verification/onchain.json") as Promise<{
    ...
    const profiles = structuredClone(sourceProfiles);
    const drifted = profiles["45650"] as { agent: { name: string; description: string; services: string } };
    drifted.agent.name = "Grid trading candidate";
    drifted.agent.description = "Grid trading with deterministic grid levels.";
    drifted.agent.services = JSON.stringify([{
      name: "MCP",
      endpoint: "https://fixture.invalid/mcp/grid",
      version: "2025-06-18",
      tools: ["createGrid", "cancelOrder"],
      capabilities: ["tools"],
    }]);
    const report = await buildBscMarketplaceReadinessReport({
      provider: new Trust8004Provider({
    ...
    const candidate = report.candidates.find((entry) => entry.agentId === "45650")!;
    expect(candidate.categories).toEqual(["rebalancing"]);
    expect(candidate.profileDerivedCategories.map((entry) => entry.category)).toEqual(["grid_trading"]);
    expect(report.categories.grid_trading.agentIds).toEqual([]);
  });
