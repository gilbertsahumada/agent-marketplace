...
import type { MarketplaceDataSort } from "@/src/data/repositories/marketplace-agent-repository";

export const dynamic = "force-dynamic";
export const metadata: Metadata = { title: "BSC agents" };
const SUPPORTED_SORTS = new Set<MarketplaceDataSort>(["newest", "reputation", "trust_score", "agent_id"]);
...
export default async function AgentsPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const sort = typeof params.sort === "string" && SUPPORTED_SORTS.has(params.sort as MarketplaceDataSort)
    ? params.sort as MarketplaceDataSort
    : undefined;
