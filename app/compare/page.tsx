...
import { compareMarketplaceAgents, getMainnetJobProof, getWorkerObservations } from "@/src/business/composition";
...
export default async function CompareRoute({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [mainnetProof, observations] = await Promise.all([
    Promise.resolve(getMainnetJobProof.execute()),
    getWorkerObservations(),
  ]);
  return <ComparePage comparison={comparison} observations={observations} selected={selected} {...(mainnetProof ? { provenAgentId: mainnetProof.agentId } : {})} />;
}
