...
import { catalogHireNetwork } from "@/src/mainnet/catalog-hire-network";
...
export async function GET(request: Request, context: { params: Promise<{ agentId: string; jobId: string }> }) {
  ...
  try {
    ...
    const target = await resolveCatalogHireTarget(agentId, quoteRequestId, { allowExpired: true, chainId });
