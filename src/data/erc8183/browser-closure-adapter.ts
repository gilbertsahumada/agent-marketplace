...
const abi = parseAbi(["function dispute(uint256 jobId)", "function settle(uint256 jobId, bytes evidence)", "function claimRefund(uint256 jobId)"]);
...
export async function executeBrowserClosure(input: { provider: unknown; wallet: string; jobId: string; action: ClosureAction; mode: "send" | "resume"; network?: "mainnet" | "testnet" }) {
  ...
  if (input.action === "refund" && input.network !== "testnet") throw new Error("Refund controls are limited to Testnet");
  ...
  const address = input.action === "refund" ? pins.commerce : input.action === "dispute" ? pins.policy : pins.router;
  const data = input.action === "refund" ? encodeFunctionData({ abi, functionName: "claimRefund", args: [id] }) : input.action === "dispute" ? encodeFunctionData({ abi, functionName: "dispute", args: [id] }) : encodeFunctionData({ abi, functionName: "settle", args: [id, "0x"] });
  ...
  const port: ClosurePort = {
    ...
    read: async () => {
      ...
      return { status: statuses[job.status] ?? "UNKNOWN", buyer: job.client, supported: (input.action !== "refund" || job.id === id) && isAddressEqual(job.evaluator, pins.router) && isAddressEqual(boundPolicy, pins.policy), disputed, verdict: verdict[0], now: block.timestamp, reviewEndsAt: job.submittedAt + window, deadline: job.expiredAt };
    },
    ...
    verify: async (hash, replacement) => {
      ...
      if (input.action === "refund") return job.id === id && job.status === 5 ? "confirmed" : "pending";
