import "server-only";
import { getCatalogCandidatePageResult } from "../data/observation/catalog-candidate-feed";
...
type Readers = { read: typeof getCatalogCandidatePageResult; directory: typeof listMarketplaceAgents.execute };
const defaults: Readers = { read: getCatalogCandidatePageResult, directory: input => listMarketplaceAgents.execute(input) };
...
export async function readCatalogResource<K extends CatalogResourceName>(resource: K, query: CatalogQuery, fresh: boolean, readers: Readers = defaults): Promise<CatalogReadResult<CatalogResourceMap[K]>> {
  ...
  try {
    if (resource === "results") {
      ...
      const read = await readers.read({ ...base, page: 1, limit: 1, includeFacets: true });
      result = read.ok ? read.data.facets ? { ok: true, data: read.data.facets } : { ok: false, error: { kind: "invalid_response" } } : read;
    } else {
      const readScope = (scope: "hiring" | "evaluation") => readers.read({ chainId, scope, statuses: [], page: 1, limit: 1, fresh });
      const [hiring, evaluation] = await Promise.all([readScope("hiring"), readScope("evaluation")]);
      result = !hiring.ok ? hiring : !evaluation.ok ? evaluation : { ok: true, data: { hiring: hiring.data.total, evaluation: evaluation.data.total } };
    }
