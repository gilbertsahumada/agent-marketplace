...
describe("Commerce indexer configuration", () => {
  it("only enables the indexer explicitly and sizes it per plan", () => {
    expect(() => loadConfig({ COMMERCE_INDEX_ENABLED: "1" })).toThrow(/COMMERCE_INDEX_ENABLED/);
  ...
  it("counts the extra cron messages in the Free queue projection", () => {
    expect(() => loadConfig({ COMMERCE_INDEX_ENABLED: "1" })).toThrow(/Free safety ceiling/);
    expect(loadConfig({ COMMERCE_INDEX_ENABLED: "0" }).projectedDailyBudget).toMatchObject({
  ...
  it("holds only an enabled indexer to the Paid D1 envelope", () => {
    ...
    expect(() => loadConfig({ ...paid, COMMERCE_INDEX_LOGS_PER_RUN: "20" })).toThrow(/D1_ROWS_WRITTEN_PER_RUN/);
